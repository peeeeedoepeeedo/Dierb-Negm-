...
export function StartMessage({recipientId,subject,returnTo}:{recipientId:string;subject:string;returnTo:string}){
 ...
 async function submit(e:FormEvent<HTMLFormElement>){
  ...
  if(r.status===401){window.location.assign(`/login?returnTo=${encodeURIComponent(returnTo)}`);return}
  const data=await r.json();
  if(r.ok)window.location.assign("/messages");else{setMessage(data.error||"تعذر إرسال الرسالة");setBusy(false)}
 }
